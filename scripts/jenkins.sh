...
ssh root@node1 "mkdir -p /project/min-tiktok/k8s"
ssh root@node2 "mkdir -p /project/min-tiktok/k8s"
cp ./k8s/* /project/min-tiktok/k8s
scp -r ./k8s/* root@node1:/project/min-tiktok/k8s || { echo "Failed to scp files"; exit 1; }
scp -r ./k8s/* root@node2:/project/min-tiktok/k8s || { echo "Failed to scp files"; exit 1; }
echo "scp done ..."
